use entropy::{Coordinates, Meetup, MeetupResult, ScraperError, ScraperMessage, ScraperResult};
use reqwest;
use std::sync::Arc;
use tokio::{
    self,
...
async fn main() {
    ...
    let (tx, mut rx): (
        Sender<ScraperMessage>,
        Receiver<ScraperMessage>,
    ) = mpsc::channel(1024);
    ...
    while let Some(msg) = rx.recv().await {
        match msg {
            ...
            ScraperMessage::ResultItem(item) => {
                match item {
...
async fn search_groups_of_chandigarh(
    meetup: Arc<Meetup>,
